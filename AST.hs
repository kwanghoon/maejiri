...
-- Contexts                  
type Ctx = (ConstTypeEnv, ConstTermEnv, DefEnv, VarTermEnv, Level)

type ConstTypeEnv = [(X, K)]
type ConstTermEnv = [(X, A)]
type DefEnv       = [(X, M)]
type VarTermEnv   = [(X, A)]
...
boundVars :: Ctx -> [X]
boundVars ctx = fst $ unzip $ for5 $ ctx

type Level = Int -- binder levels

toCtx sigs = (typeDecls, termDecls, defDecls, [], 0)
  where
    typeDecls = [(s,toDBIdxK k) | HasKind (ConstA s) k <- sigs]
    termDecls = [(s,toDBIdxA t) | HasType (ConstM s) t <- sigs]
    defDecls  = [(s,toDBIdxM m) | HasDef  (ConstM s) m <- sigs]

addtermtype :: Ctx -> X -> A -> Ctx
addtermtype (cte1, cte2, de, vte, level) x a 
  = (cte1, (x,a):cte2, de, vte, level)

addtype :: Ctx -> X -> A -> Ctx
addtype (cte1, cte2, de, vte, level) x a 
  = (cte1, cte2, de, (x,a):vte, level)

inclevel :: Ctx -> Ctx
inclevel (cte1, cte2, de, vte, level) 
  = (cte1', cte2', de', vte', level')
  where
    cte1'  = cte1
    cte2'  = cte2
    de'    = de
...
sigmakindof s (tyctx, _, _, _, _) =
  case [ k | (t,k) <- tyctx, s==t ] of
    [k] -> Just k
    _   -> Nothing
    
sigmatypeof s (_, tmctx, _, _, _) =
  case [ t | (m,t) <- tmctx, s==m ] of
    [t] -> Just t
    _   -> Nothing
    
typeof i (_, _, _, vtenv, _) =
  if length vtenv > i
  then Just $ snd $ head (drop i vtenv)
  else Nothing
       
defof s (_, _, denv, _, _) =
  case [ m | (x,m) <- denv, s==x ] of
    [m] -> Just m
    _   -> Nothing
